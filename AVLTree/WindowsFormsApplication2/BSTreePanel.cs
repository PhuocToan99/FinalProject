...
namespace WindowsFormsApplication2
{
    partial class BSTreePanel : UserControl
    {
        ...
        //
        public void NormalAdd(int size, int min, int max)
        {
            if (min >= max || (max - min < size))
            {
                MessageBox.Show("Invalid parameters!", "OMG");
        ...
        {
            _ratio = (_Tree.Count * 400) / this.Width;// +RADIUS / this.Width;
        ...
        private void CalculateSize(Graphics g, float left, AVLTreeNode<int> node)
        {
            if (node != null)
            {
        ...
        public bool AddNode(int value)
        {
            ...
            if (_Tree.Check(value)==false)
            {
                MessageBox.Show("Tree does not contain value " + value, "Add Error", MessageBoxButtons.RetryCancel,MessageBoxIcon.Hand);
                return false;
            }
        ...
        public bool SearchNode(int value)
        {
            ...
            if (_queue == null)
            {
                MessageBox.Show("Tree does not contain value " + value, "Search", MessageBoxButtons.RetryCancel);
                return false;
            }
            txtOutput.Clear();
            BeginDraw();
        ...
        {
            if (_Tree.CheckRemove(value)==false)
            {
                MessageBox.Show("Tree does not contain value " + value, "Delete Error", MessageBoxButtons.RetryCancel, MessageBoxIcon.Hand);
                return false;
            }
            txtOutput.Clear();
            BeginDraw();
